...
export function SaInit({server_url, pro_name, user_id = '', gps_lon = 0, gps_lat = 0} = {}) {
  sa.init({
...
let second = 0
let timer
let debounceFlag // 防抖标识位
function debounce(dom, bottom) {
  if (debounceFlag) clearTimeout(debounceFlag)
    debounceFlag = setTimeout(() => {
      ...
      debounceFlag = undefined
    }, 2000)
}
export function stayTime(id, height, top) {
  let dom = document.getElementById(id)
  let bottom = height + top
  document.addEventListener('scroll', debounce(dom, bottom), false)
}
