...
const VIEWPORT_AREA: number = WW * WH;

const DELAY: number = 500;

export class FMPCollect {
  statusCollector: any[] = [];
  flag: boolean = true;
  ...
  doTag(target: any, callbackCount: number) {
  ...
  calFinallScore(): void {
    if (MutationObserver && this.flag) {
      if (!this.checkCanCal(START_TIME)) {
        ...
        let res: any = this.deepTraversal(document.body);

        let tp: any;

        res.dpss.forEach((item: { st: number; }) => {
          if (tp && tp.st) {
            if (tp.st < item.st) {
              tp = item;
            }
